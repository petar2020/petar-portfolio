...
import { CONTENT_DATE } from '../data/comparisons'
import { useTranslations } from 'next-intl'
import { useRouter } from 'next/router'
import SEO from './SEO'
import SubpageNav from './SubpageNav'
import Footer from './Footer'
import { RelatedServiceCard } from './ServicePageTemplate'
import { getServiceBySlug } from '../data/services'

const SITE_URL = 'https://petararsic.rs'
const OG_LOCALE_MAP = { sr: 'sr_RS', en: 'en_US' }
const CASE_STUDIES = {
  drivesoft: { namespace: 'caseStudyPage', summary: 'caseStudy', related: ['booking-systems', 'admin-panels-dashboards', 'laravel-applications'], demos: [{ href: 'https://demo.drivesoft.rs/passenger-demo', key: 'ctaPassenger' }, { href: 'https://demo.drivesoft.rs/admin-demo', key: 'ctaAdmin' }] },
  'leto-halkidiki': { namespace: 'caseStudyPages.letoHalkidiki.body', summary: 'caseStudyPages.letoHalkidiki.summary', related: ['tourism-accommodation-portals', 'laravel-applications', 'website-redesign'], article: true, demos: [{ href: 'https://letohalkidiki.com/sr', key: 'ctaLive' }] },
}

export default function CaseStudyPage({ locale, caseStudyKey = 'drivesoft' }) {
  const config = CASE_STUDIES[caseStudyKey]
  const t = useTranslations(config.namespace)
  const tSub = useTranslations('subpage')
  const tCS = useTranslations(config.summary)
  ...
  const relatedServices = config.related.map((slug) => getServiceBySlug(slug)).filter(Boolean)

  const pageUrl = `${SITE_URL}/${currentLocale}/case-study/${caseStudyKey}`
  ...
  return (
    <>
      <SEO
        ...
        isMainPage={false}
        hrefLangPath={`/case-study/${caseStudyKey}`}
        type={config.article ? 'article' : 'website'}
        additionalSchemas={config.article ? [breadcrumbSchema, {
          '@context': 'https://schema.org', '@type': 'Article', '@id': pageUrl + '#article',
          headline: tCS('title'), description: t('seo.description'), url: pageUrl, mainEntityOfPage: pageUrl,
          inLanguage: currentLocale, datePublished: CONTENT_DATE, dateModified: CONTENT_DATE,
          author: { '@type': 'Person', '@id': SITE_URL + '/#person', name: 'Petar Arsić', url: SITE_URL },
          publisher: { '@type': 'Person', '@id': SITE_URL + '/#person' }, image: SITE_URL + '/letohalkidiki-preview.jpg',
        }] : [breadcrumbSchema]}
      />
      ...
      <main className="pt-20">
        ...
        <section className="grain bg-ink-900 border-b border-line py-16 sm:py-24">
          <div className="container mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
            <div className="flex items-center gap-3 mb-6">
              <span className={`inline-block h-1.5 w-1.5 rounded-full bg-amber ${config.article ? '' : 'animate-pulse'}`} />
            ...
            {config.article && <p className="mt-5 text-sm text-paper-dim">{t('author')}: <a href={`/${currentLocale}`} rel="author" className="text-teal-bright underline">Petar Arsić</a> · {t('updated')}: <time dateTime={CONTENT_DATE}>{currentLocale === 'sr' ? '23. 9. 2026.' : '23 September 2026'}</time></p>}
          ...
          <div className="container mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              {config.demos.map((demo, i) => (
                <a key={demo.key} href={config.article ? `https://letohalkidiki.com/${currentLocale}` : demo.href} target="_blank" rel="noopener noreferrer" className={i === 0 ? 'btn-signal' : 'btn-line'}>
                  {t(demo.key)} <span aria-hidden>→</span>
                </a>
              ))}
            </div>
          </div>
        ...
        {config.article && (
          <section className="bg-ink-900 border-b border-line py-12">
            <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
              <RelatedPages title={t('relatedTitle')} locale={currentLocale} ids={['apartments', 'travel-agency', 'wordpress-vs-custom']} />
            </div>
          </section>
        )}
        ...
        <section className="grain bg-ink-900 py-16 sm:py-20">
          <div className="container mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
            ...
            <a href={config.article ? `/${currentLocale}/${currentLocale === 'sr' ? 'kontakt' : 'contact'}` : `/${currentLocale}#contact`} className="btn-signal">
